...
public class InventoryTest {
    ...
    */
	...
	Inventory inv = new Inventory("jdbc:mysql://localhost/inventory","scm","ensf409");
        ...
        furn = furn.toLowerCase();
        if (inv.isValidFurniture(furn) == true && inv.isValidQuantity("1")) {	
            type = type.toLowerCase();
            if (furn == "chair"){
                if (type == "kneeling"){
                    checks = true;
        ...
        assertEquals("Order could not be made since the type of furntiture does not exist", checks, true);
   }
